...
def inicializa_main_tres_itens():
    ROOT = 0

    # Fazendeiro, alface, bode, lobo:
    dicionario_heuristica_3_itens = {
        'A B F L|': 22,
        'A L|B F': 18,
        'A F L|B': 17,
        'L|A B F': 14,
    ...
    vet_dist_estado_final = converteDicToList(dicionario_heuristica_3_itens)
    ...
    caminho_sem_no_final_e_sem_no_inicial = list(
        set(caminho_busca_largura) - set(caminho_busca_largura).intersection(set(nos_finais)) - set(
            'A B F L|'))  # caminho sem o no final

    # pinta de verde o caminho encontrado pela busca em largura, de vermelho o no final e de azul os demais nos
    node_colors = [
        'purple' if n == 'C F G Q R|' else 'yellow' if n in nos_finais else 'green' if n in caminho_sem_no_final_e_sem_no_inicial else 'blue'
        for n in g.nodes()]
